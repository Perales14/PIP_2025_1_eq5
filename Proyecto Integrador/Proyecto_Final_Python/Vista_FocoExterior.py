...
qtCreatorFile = "Vista_FocoExterior.ui"
...
class MyApp(QtWidgets.QMainWindow, Ui_MainWindow):
    def __init__(self, parent=None):
        ...
        self.ldr_dato = 0  # Valor de 0 a 4096
        self.estado = 0  # 0: apagado, 1: encendido
        self.umbral = 3000  # Umbral para encender el foco
    ...
    def actualizar_datos(self):
        ...
        if self.parent and hasattr(self.parent, 'ldr_dato'):
            # Si hay referencia a la ventana principal, obtener datos de ahí
            self.ldr_dato = self.parent.ldr_dato
            self.estado = self.parent.foco_exterior_estado
            if hasattr(self.parent, 'historial_foco_exterior'):
                self.historial = self.parent.historial_foco_exterior.copy()
        else:
            # En caso contrario, simular localmente
            import random
            self.ldr_dato = random.randint(0, 4096)
            self.estado = 1 if self.ldr_dato >= self.umbral else 0

            # Actualizar historial
            self.historial.append(self.ldr_dato)
    ...
    def mostrar_datos(self):
        ...
        self.listWidget.addItem(f"Nivel de luz (LDR): {self.ldr_dato}")
        self.listWidget.addItem(f"Umbral: {self.umbral}")
        self.listWidget.addItem(f"Estado: {'Encendido' if self.estado else 'Apagado'}")

        # Agregar información adicional
        if self.ldr_dato >= self.umbral:
            self.listWidget.addItem("El nivel de luz ha superado el umbral")
        else:
            self.listWidget.addItem("El nivel de luz está por debajo del umbral")

    def prender_apagar(self):
        if self.btn_Prender.text() == "Prender":
            self.estado = 1
            self.parent.foco_exterior_estado = 1
            self.parent.focoxt = False
            self.btn_Prender.setText("Apagar")
        else:
            self.estado = 0
            self.parent.foco_exterior_estado = 0
            self.parent.focoxt = True
    ...
    def visualizar_grafica(self):
        ...
        if not self.ventana_grafica:
            self.ventana_grafica = Vista_Grafica.MyApp(tipo_sensor="foco_exterior", datos=self.historial.copy())
        else:
            # Actualizar los datos si ya existe
            self.ventana_grafica.actualizar_datos(self.historial.copy())
            self.ventana_grafica.tipo_sensor = "foco_exterior"
