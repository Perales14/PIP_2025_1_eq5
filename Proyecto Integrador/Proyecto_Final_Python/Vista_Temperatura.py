...
qtCreatorFile = "Vista_Temperatura.ui"
...
class MyApp(QtWidgets.QMainWindow, Ui_MainWindow):
    def __init__(self, parent=None):
        ...
        self.temperatura = 25  # Temperatura en grados Celsius
        self.umbral = 45  # Umbral para encender el abanico
    ...
    def actualizar_datos(self):
        ...
        if self.parent and hasattr(self.parent, 'temperatura_dato'):
            # Si hay referencia a la ventana principal, obtener datos de ahí
            self.temperatura = self.parent.temperatura_dato
            self.estado = self.parent.abanico_estado
            if hasattr(self.parent, 'historial_temperatura'):
                self.historial = self.parent.historial_temperatura.copy()
        else:
            # En caso contrario, simular localmente
            import random
            self.temperatura += random.uniform(-0.5, 0.5)
            self.temperatura = max(15, min(35, self.temperatura))  # Mantener entre 15 y 35 grados
            self.estado = 1 if self.temperatura > self.umbral else 0

            # Actualizar historial
            self.historial.append(self.temperatura)
    ...
    def mostrar_datos(self):
        ...
        self.listWidget.addItem(f"Temperatura actual: {self.temperatura:.1f} °C")
        self.listWidget.addItem(f"Umbral: {self.umbral} °C")
        self.listWidget.addItem(f"Estado del abanico: {'Encendido' if self.estado else 'Apagado'}")

        # Agregar información adicional
        if self.temperatura > self.umbral:
            self.listWidget.addItem("La temperatura supera el umbral, abanico encendido")
        else:
            self.listWidget.addItem("La temperatura está por debajo del umbral, abanico apagado")

    def prender_apagar(self):
        if self.btn_Prender.text() == "Prender":
            self.estado = 1
            self.parent.abanico_estado = 1
            self.parent.abanico = False
            self.btn_Prender.setText("Apagar")
        else:
            self.estado = 0
            self.parent.abanico_estado = 0
            self.parent.abanico = True
    ...
    def visualizar_grafica(self):
        ...
        if not self.ventana_grafica:
            self.ventana_grafica = Vista_Grafica.MyApp(tipo_sensor="temperatura", datos=self.historial.copy())
        else:
            # Actualizar los datos si ya existe
            self.ventana_grafica.actualizar_datos(self.historial.copy())
            self.ventana_grafica.tipo_sensor = "temperatura"
